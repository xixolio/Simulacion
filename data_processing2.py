# -*- coding: utf-8 -*-
"""
Created on Tue Oct 16 17:29:51 2018
...
def get_missing_values(path,name):
    ...
    temporal_data = pd.read_csv(path+name,usecols=[0,4,7,9,10],encoding='utf-8')
    temporal_data.columns = ['date','speed','direction','humidity','temperature'] 
...
#            print(month)
    ...
    for i in range(len(temporal_data)):
        ...
        direction = temporal_data['direction'][i]
        humidity = temporal_data['humidity'][i]
        temperature = temporal_data['temperature'][i]
...
#                print(expected_date[e_index])
        ...
        if  math.isnan(float(speed)) or speed is None or \
        math.isnan(float(direction)) or direction is None or \
        math.isnan(float(humidity)) or humidity is None or \
        math.isnan(float(temperature)) or temperature is None:
...
def get_data(path, name, ts=1, lag=1, overlap=True, output=1):
    
    #path = ''
    data = pd.read_csv(path+name, usecols = ['date','direction','temperature','humidity','speed'])
    ...
    data['x'] = data['speed']*np.cos(data['direction']/(2*np.pi))
    data['y'] = data['speed']*np.sin(data['direction']/(2*np.pi))
    ...
    for i in range(number_of_sets):
        ...
        min_speed = temp_data[:-24].values.min(axis=0)
        max_speed = temp_data[:-24].values.max(axis=0)
    ...
    for i in range(number_of_sets):
        ...
        shifted_sets = [data_sets[i].shift(lag+output-1-k) for k in range(lag+output)]
        
        temp_input = pd.concat(shifted_sets,axis=1).dropna()
        temp_output = temp_input.iloc[:,-output*4:]
        temp_input = temp_input.iloc[:,:-output*4]
        ...
        training_data_input.append(temp_input[:-24].values.reshape(-1,ts,lag*4))
        testing_data_input.append(temp_input.iloc[-24:].values.reshape(-1,ts,lag*4))
