...
for i,params in enumerate(parameters):
    ...
    #simulated_data = periodic_simulate(n_simulations,simulation_length,wind_input[0],0,model,model_name)
    
    simulated_data = np.loadtxt('simulations/'+model_name+'.txt')
    mean_hist, mean_acf, mean_pacf = metrics_and_plots(wind_data[:2000], simulated_data)
    ...
    np.savetxt('simulation_results2/'+model_name+'.txt', np.array(results))
